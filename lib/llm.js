// lib/llm.js
// Provider switch via env: LLM_PROVIDER=openai | gemini
...
let openai = null;
let genAI = null;
if (PROVIDER === "openai") openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
if (PROVIDER === "gemini") genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

function buildPrompt({ clues, gridState, solvedKeys }) {
  const revealed = Object.entries(gridState || {})
    .filter(([_, v]) => v && typeof v === "object" && v.value)
